...
@Component({
  selector: 'app-icon-qty-product-reclamation-rt',
  templateUrl: './icon-qty-product-reclamation-rt.component.html',
  styleUrls: ['./icon-qty-product-reclamation-rt.component.css']
})
export class IconQtyProductReclamationRtComponent implements OnInit {
      ...
      this.rtService.getAllReclamationsById(this.userId).subscribe(
        (Response) => {
          ...
          for (let i = 0; i < this.reclamationsTab.length; i++) {
            this.allQty=this.allQty+this.reclamationsTab[i].qty;
