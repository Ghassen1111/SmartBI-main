...
@Component({
  selector: 'app-add-tache-employeur-rl',
  templateUrl: './add-tache-employeur-rl.component.html',
  styleUrls: ['./add-tache-employeur-rl.component.css']
})
export class AddTacheEmployeurRlComponent implements OnInit {
  fullDate: String;
  employeursTab: any = [];
  id: any;
  tacheForm: FormGroup;
  ...
  constructor(private rlService: ResponsableLogistiqueService, private activateRoute: ActivatedRoute, private router: Router, private formBuilder: FormBuilder, private tacheService: TacheService) { }
  ...
  ngOnInit() {
    ...
    this.rlService.getAllEmployeursById(entrepriseId).subscribe(
      (Response) => {
    ...
    this.tacheService.addTache(this.tacheForm.value).subscribe(
      (Response) => {
        ...
        if (Response.message == "Email exsist") {
          ...
          this.router.navigate([`addTacheEmployeurRl/${this.id}`]);
        }
        this.tacheService.getMyAddTachesById(this.id).subscribe(
          (Response) => {
            this.tachesTab = Response.taches;
